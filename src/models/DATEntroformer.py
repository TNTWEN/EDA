...
class AttentionBlock(nn.Module):
    ...
    def forward(
        ...
        hidden_states = hidden_states + self.dropout(attention_output)#fix
    ...
    def build(self):
        ...
        for i in range(self.scale+1):
            block_scale = nn.ModuleList(
                    [Block(self.config,self.ksizes[i],self.strides[i])for _ in range(num_each_stage)]
                )
...
class TransDecoder(nn.Module):
    ...
    def build(self):
        ...
        self.blocks = nn.ModuleList(
            [Block(self.config,self.ksizes,self.strides) for _ in range(self.config.num_layers)]
        )
...
class DATEntroformer(CompressionModel):
